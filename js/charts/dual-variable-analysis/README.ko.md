...
각 물리량의 점 값은 지표 패널이 그 물리량에 쓰는 계열에서 옵니다(선택한 구간에 대한 `computeSectorMetrics` —— 패널 자신의 범위입니다. 경계가 두 트랙 점 사이에 놓이면 지표와 똑같이 보간됩니다). 차트가 패널보다 더 정직하거나 덜 정직할 수 없습니다. 그려진 값이 패널의 통계와 어긋나거나 그것을 넘어설 수 없습니다. 그 값이 바로 그 통계가 계산되는 계열이기 때문입니다. 물리량별로는:

1. **심박 / 케이던스 / 파워** — 패널의 피트니스 계열: 유한한 판독값에서 확정된 일시 정지 구간 안의 타임스탬프를 제거하고(`pauseFreeSamples`와 같은 판정 — 마지막 이동 시점은 제외, 정지 종료 시점은 포함. 구간은 공용 `createPauseTracker`에서 가져오며 모듈 고유의 감지기는 쓰지 않음), 남은 컴팩트한 목록에 공용 5점 슬라이딩 윈도우 평활을 적용합니다(`avgHr/maxHr`, `avgCad/maxCad`, `avgPower/maxPower`의 배후에 있는 배열 그 자체). 제거되거나 결측인 판독값은 NaN으로 남습니다.
2. **온도** — 날것 그대로의 판독값. 필터하지 않고 일시 정지 중의 판독값도 포함합니다. 패널의 `rawStats`와 같습니다(환경 판독값은 노력 신호가 아닙니다).
3. **속도 / 페이스 / GAP** — 구간에 대한 패널의 '최고 속도' 계열. 구간 자신의 점 위에서 범위 국소적으로 구축합니다(`maxCleanedPointSpeed`의 줄마다 미러: 기록값 → dd/dt 교차 검증 → 5점 윈도우. 계산값 → 3σ. 규칙 선택은 프로필의 `buildCaches`처럼 전역 trackHasRecordedSpeed 판정을 따름). 평활 윈도우가 범위 국소적이므로, 하위 구간에서 그려진 최고 속도는 그 범위에 대한 패널의 최고 속도와 같습니다. 구간 가장자리 근처에서는 프로필의 트랙 전체 속도 곡선과 (약간) 다를 수 있습니다. 일시 정지 점은 남습니다(휴식 중의 0도 포함). GAP는 그 속도를 그 점을 떠나는 날것의 구간별 경사의 Minetti 인자로 나눈 것입니다(프로필의 GAP 오버레이와 같음).
4. **경사** — 구간에 대한 패널의 경사 윈도우(보간된 경계 포함): 수평 거리가 `GRADIENT_WINDOW_M`(50 m)에 쌓이면 그 윈도우의 승강비가 윈도우의 닫는 점에 놓이고 다음 윈도우가 거기서 시작합니다. 끝의 50 m 미만 잔여 창은 길이 ≥ 20 m이면 계상됩니다. 유한한 값은 `maxGrade/minGrade`가 계산하는 집합 그 자체이며, 차트에서 가장 가파른 경사가 패널의 최대 경사입니다. 경사 점은 그래서 드문듯합니다(약 50 m당 하나). 경사와의 쌍은 윈도우 닫는 점의 행만 남깁니다.
5. **고도** — 날것 그대로의 점 고도. `eleMin/eleMax`와 같습니다.
6. **결측값** — X나 Y가 유한하지 않은 쌍은 빈닝에 들어가지 않습니다(0 채우기, 이전 값 복사, 보간 모두 없음).

원래 명세의 행 수준 파이프라인(행 전체 일시 정지 제거와 첫 버전 §10의 무효 파워 / 무효 케이던스 제외)은 2026-09-17에 명시적 요청으로 제거되었습니다: 패널은 그런 규칙을 적용하지 않습니다 —— 0 W 코스팅 판독값은 평균에 포함되고 온도는 일시 정지 중 판독값을 유지합니다 —— 그러므로 차트도 그것들을 버려서는 안 됩니다. 명세의 §10(과 §20의 이상값 입장)은 같은 날 이 규칙으로 다시 쓰였습니다. 제외는 물리량별로 패널의 메커니즘이 제외하는 바로 그 지점에서 일어나며, 점이 분석을 떠나는 경로는 규칙 6뿐입니다. 패널 자신의 집계 전용 통계(구간별 dd/dt에 의한 평균 속도 / 평균 페이스, 구간 노력 페이스에 의한 평균 GAP)는 여기서 대응하는 점 단위 값이 없으며, 차트 자신의 평균도 없습니다.
...
`index.js`가 구독하는 것: `language:changed`(열린 뷰 다시 그리기), `units:changed`(눈금 다시 그리기), `theme:changed`(LUT 재구축 + 다시 그리기), `trackStore`(샘플 캐시 무효화, 대화상자 닫기, 버튼 전환), `sectorStore`(샘플 캐시 무효화 —— 분석은 지표 패널처럼 선택한 구간을 읽으며, 다음 열기/분석이 새 범위로 다시 구축합니다). 대화상자는 모달이라 열려 있는 동안 구간은 움직일 수 없으므로, 구간 변화는 실제로는 세션 사이에 도착합니다. 이 구독들을 제거하지 마세요.

## 검증

`tests/suite-dualVariable.js`는 조합 테이블, 물리량별 패널 정합(일시 정지 제거, 5점 평활, 코스팅 0 유지, 일시 정지 포함 온도, 50 m 경사 윈도우), 구간 범위(선택 구간 밖의 행은 모든 물리량에서 NaN. 보간된 경계를 가진 트랙 중간 범위에 대한 `computeSectorMetrics`와의 정합. 트랙 전체 일시 정지 안에서 시작된 구간 포함), 패널 정합 불변식(각 물리량의 유한한 점에 대한 차트의 최대/평균/최소가 패널의 수치와 정확히 일치), NaN 처리, 가용성, 쌍 추출, 밀도 그리드(강건한 도메인, 퇴화한 범위, bin 일관성), tooltip 배치 사다리를 다룹니다. 7개 스위트 총 42개 케이스. 로컬 HTTP 서버의 `tests/index.html`에서 전체 스위트를 실행하세요. 전부 통과를 유지하고, 새 데이터 규칙을 추가할 때 함께 확장합니다. 공용 제스처는 따로 검증합니다. `tests/suite-viewport.js`가 창 계산(클램프, 축별 방향 규칙, 앵커 유지, 두 번 탭 규칙)을 고정합니다. 포인터 경로 자체는 **수동** 확인입니다 — 거친 포인터 기기나 기기 에뮬레이션에서(터치 제스처는 거기서만 존재합니다): 핀치와 두 손가락 드래그로 확대/이동, 한 손가락 스와이프로 읽기, 두 번 탭으로 전체 범위 복귀, 그리고 창을 움직이는 제스처가 고정된 읽기를 버리는지(더 이상 같은 데이터를 가리키지 않는 화면 위치에 남기지 않는지)를 확인합니다.
